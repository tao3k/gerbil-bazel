...
(def string-trim-both string-trim)
(def (string-tokenize value)
  (filter (lambda (part) (not (string=? part "")))
          (string-split
           (string-map (lambda (char) (if (char-whitespace? char) #\space char)) value)
           #\space)))
...
(def (receipt-json receipt)
  (json->string receipt sort-keys: #t))
