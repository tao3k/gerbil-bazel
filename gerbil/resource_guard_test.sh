...
env \
  ...
  -u GERBIL_BAZEL_GUARD_MAX_RSS_BYTES \
  GERBIL_BAZEL_GUARD_CGROUP_MEMORY_LIMIT_BYTES=8589934592 \
  GERBIL_BAZEL_GUARD_CGROUP_MEMORY_CURRENT_BYTES=6442450944 \
  GERBIL_BAZEL_GUARD_RSS_HEADROOM_BYTES=1073741824 \
  GERBIL_BAZEL_GUARD_RUNNABLE_PROCESSES=1 \
  "GERBIL_BAZEL_GUARD_PROCESS_TABLE_SNAPSHOT=1 0 0" \
  "$gxi" "$guard" "$root/cgroup-remaining-memory.json" \
  cgroup-remaining-memory 5 \
  /bin/sh -c 'exit 0'
grep -F '"systemMemoryBytes":8589934592' \
  "$root/cgroup-remaining-memory.json" >/dev/null
grep -F '"availableMemoryBytes":2147483648' \
  "$root/cgroup-remaining-memory.json" >/dev/null
grep -F '"maxRssBytes":1073741824' \
  "$root/cgroup-remaining-memory.json" >/dev/null
